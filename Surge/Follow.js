...
function getPower(callback) {
  $httpClient.get({
    url: $persistentStore.read('Follow_URL'), headers: {'cookie': 'authjs.session-token=' + `${$persistentStore.read('Follow_sToken')}`}
...
function postRequest() {
  $httpClient.post({
    ...
    headers: {'cookie': 'authjs.session-token=' + `${$persistentStore.read('Follow_sToken')}` + ';' + `${$persistentStore.read('Follow_CSRF')}`},
